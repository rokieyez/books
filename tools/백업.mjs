 ...
 *   → 사진은 **서재의 작업대**에서 「사진을 내려받는다 (ZIP)」로 받아 이 폴더의
 *     사진/ 에 푼다. 나머지는 대시보드에서. 자세한 것은 적바림.md 에 적힌다.
 *
 * 쓰는 법:
...
받아야 합니다. **사진 하나만 챙기면 이 백업은 온전합니다** — 나머지 둘은 겹칩니다.

| 무엇 | 겹치나 | 어떻게 받나 |
|---|---|---|
| **intake 버킷의 책장 사진** | **안 겹칩니다 — 이것만 잃으면 되찾을 수 없습니다** | 서재 → 로그인 → 작업대의 「장서를 베껴 둔다」 → **사진을 내려받는다 (ZIP)**. 받은 꾸러미를 이 폴더에 풀면 됩니다:<br>\`unzip 서가뒤의방-사진-*.zip -d 사진/\`<br>꾸러미 안에 \`intake_photos.json\`(어느 사진에서 몇 권을 읽었나)이 함께 들어 있습니다 |
| \`intake_candidates\` | AI 가 읽은 내력. 확정된 것은 \`books\` 에 반영돼 겹칩니다 | 대시보드 → Table Editor → CSV |
| \`backup_snapshots\` | \`db/books.json\` 과 겹칩니다 | 대시보드 → Table Editor |
